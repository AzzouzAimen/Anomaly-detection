#!/usr/bin/env python3
"""Load Sprint 2 ETL outputs into TimescaleDB via direct psycopg2 connection."""
...
DEFAULT_HOST = env_default("TIMESCALE_HOST", "127.0.0.1")
DEFAULT_PORT = int(env_default("TIMESCALE_PORT", "5433"))
DEFAULT_DATABASE = env_default("TIMESCALE_DB", "apnea_ts_db")
DEFAULT_USER = env_default("TIMESCALE_USER", "user")
DEFAULT_PASSWORD = env_default("TIMESCALE_PASSWORD", "timescale_password")
BATCH_SIZE = 100_000
TIMESCALE_AUTH_HELP = (
    "Verify TIMESCALE_HOST/TIMESCALE_PORT/TIMESCALE_DB/TIMESCALE_USER/TIMESCALE_PASSWORD or pass "
    "explicit --host/--port/--database/--user/--password arguments. If the container was created before the compose "
    "default-user change, recreate the Timescale service or retry with --user timescale_user --password timescale_password."
)
        ...
        for start in range(0, len(rows), BATCH_SIZE):
            ...
            if table_name == "annotations_apnea":
                ...
                total += copy_rows(
                    connection,
                    "annotations_apnea",
                    ["recording_id", "recorded_at", "minute_index", "label", "is_apnea"],
                    batch,
                )
            else:
                batch = [
                ...
                total += copy_rows(
                    connection,
                    "annotations_qrs",
                    ["recording_id", "recorded_at", "sample_index"],
                    batch,
                )
...
def enable_compression(connection) -> None:
    run_sql(
        connection,
        (
    ...
    run_sql(connection, "SELECT add_compression_policy('signals', INTERVAL '1 day', if_not_exists => TRUE);")
    run_sql(
        connection,
        (
...
def run_load(
    ...
    compress_after_load: bool = False,
    ...
    connection = connect_db(
        ...
        user,
        password,
        database_label="TimescaleDB",
        auth_help=TIMESCALE_AUTH_HELP,
    )
    ...
    try:
        ...
        if compress_after_load:
            stage_start = time.perf_counter()
            enable_compression(connection)
            connection.commit()
            stats.append(IngestionStats("compression", 1, time.perf_counter() - stage_start))
        else:
            stats.append(IngestionStats("compression", 0, 0.0))
    except Exception:
        connection.rollback()
        raise
    finally:
    ...
    write_summary_json(summary_output_path or Path("database/timeseries/ingestion_log.json"), summary)
    return summary


def main() -> None:
    parser = argparse.ArgumentParser(description="Load ETL outputs into TimescaleDB.")
    ...
    parser.add_argument(
        "--compress-after-load",
        action="store_true",
        help="Compress Timescale signal chunks immediately after load instead of leaving compression to the benchmark/storage phase.",
    )
    ...
    logger.info("Loaded TimescaleDB data: %s", summary.to_dict())
